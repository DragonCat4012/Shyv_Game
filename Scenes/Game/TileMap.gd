...
var landVariations = [land_atlas, land_atlas2]
	...
	for x in range(-width/2 , width/2):
		for y in range(-height/2, height/2):
			...
			if noiseValue >= 0.4: # high land
				set_cell(layerTerrain, Vector2(x,y), source_id, high_land_atlas)
				GamManager.land_tiles.append(Vector2(x,y))
			elif noiseValue < 0.0: # water
				set_cell(layerTerrain, Vector2(x,y), source_id, water_atlas)
				...
				for n in neighbors:
					if noise.get_noise_2d(n.x, n.y) >= 0:
						touchesLand = true
				if touchesLand:
					set_cell(layerTerrain, Vector2(x,y), source_id, water_atlas2)
